...
# 02110-1301  USA
__version__ = '0.3'
__all__ = ['opmap', 'opname', 'opcodes', 'hasflow', 'stack_effect', 'cmp_op', 'hasarg', 'hasname', 'hasjrel', 'hasjabs',
           'hasjump', 'haslocal', 'hascompare', 'hasfree', 'hascode', 'Opcode', 'SetLineno', 'Label', 'isopcode', 'Code']
import opcode
from sys import version_info
from dis import findlabels, stack_effect
...
_se = {RETURN_VALUE: -1, IMPORT_FROM: 1, DUP_TOP: 1, LOAD_CONST: 1, LOAD_NAME: 1, LOAD_GLOBAL: 1, LOAD_FAST: 1, LOAD_CLOSURE: 1, LOAD_DEREF: 1, BUILD_MAP: 1, LOAD_BUILD_CLASS: 1,
       YIELD_VALUE: 0, UNARY_POSITIVE: 0, UNARY_NEGATIVE: 0, UNARY_NOT: 0, UNARY_INVERT: 0, GET_ITER: 0, LOAD_ATTR: 0, IMPORT_NAME: 0, ROT_TWO: 0, ROT_THREE: 0, NOP: 0, DELETE_GLOBAL: 0, DELETE_NAME: 0, DELETE_FAST: 0,
...
__all__.append('se')
hasflow = hasjump | {WITH_CLEANUP, POP_BLOCK, END_FINALLY,
                     BREAK_LOOP, RETURN_VALUE, RAISE_VARARGS, STOP_CODE}


class Label:
    pass
SetLineno = type("SetLinenoType", (object,), {
                 "__repr__": lambda s: 'SetLineno'})
...
class Code(object):
    ...
    def _compute_stacksize(self):
        code = self.code
        label_pos = {op[0]: pos for pos, op in enumerate(
            code) if isinstance(op[0], Label)}
        ...
        sf_targets = {
            label_pos[arg] for op, arg in code if op == SETUP_FINALLY or op == SETUP_WITH}
        ...
        def newstack(n):
            if curstack[-1] < -n:
                raise ValueError(
                    "Popped a non-existing element at %s %s" % (pos, code[pos - 3:pos + 2]))
                ...
                if stacks[pos] is None:
                    ...
                    if code[op][0] not in (RETURN_VALUE, RAISE_VARARGS, STOP_CODE):
                        raise ValueError("Inconsistent code at %s %s %s\n%s" % (
                            pos, curstack, stacks[pos], code[pos - 5:pos + 4]))
        return maxsize
    ...
    def to_code(self):
        ...
        co_flags = (not(STORE_NAME in co_flags or LOAD_NAME in co_flags or DELETE_NAME in co_flags)) | (self.newlocals and CO_NEWLOCALS) | (
            self.varargs and CO_VARARGS) | (self.varkwargs and CO_VARKEYWORDS) | ((YIELD_VALUE in co_flags) << 5) | ((not co_flags & hasfree) << 6)
        ...
        for i, (op, arg) in enumerate(self.code):
            if isinstance(op, Label):
                ...
                if arg > 0xFFFF:
                    co_code += bytes((opcode.EXTENDED_ARG, arg >>
                                      16 & 0xFF, arg >> 24 & 0xFF))
